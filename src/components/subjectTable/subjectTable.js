...
class SubjectTableWithoutRouter extends React.Component {
  ...
  addSubject = () => {
    this.setState(
      ...
      () => {
        axios
          .post(`${process.env.REACT_APP_HOST}/subject`, this.state.subject, {
  ...
  edit = async () => {
    try {
      const { data } = await axios.put(
        `${process.env.REACT_APP_HOST}/subject/id/${this.state.subjectId}`,
        this.state.subject,
        {
          headers: { "Content-Type": "application/json" },
          responseType: "json",
    ...
    try {
      ...
      if (check) {
        const data = await axios.delete(
          `${process.env.REACT_APP_HOST}/subject/${id}`
        );
    ...
    const tableContent = this.props.subjects.map((subject, index) => {
      return (
        <tr key={subject._id}>
          ...
          <td>
            <button
              onClick={this.openModal.bind(this, index, subject._id)}
              className="btn btn-info m-1"
            >
              Edit
            </button>
            <button
              onClick={this.onDelete.bind(this, subject._id)}
      ...
      <tr>
        ...
        <td>
          <input
            ...
            type="text"
            name="name"
            style={{ border: "1px solid black", lineHeight: "25px" }}
          ></input>
        </td>
        <td>
          <select onChange={this.changeHandler} name="fullMarks">
            <option value={100}>100</option>
            <option value={50}>50</option>
          </select>
        </td>
        <td>
          <button onClick={this.addSubject} className="btn btn-warning m-1">
            Save Changes
          </button>
          <button
            onClick={() => {
        ...
        >
          ...
          <form className="form-group">
            ...
            <input
              name="name"
              onChange={this.changeHandler}
              defaultValue={this.state.currentSubjectName}
              className="form-control"
            ></input>
            <label>
              <h5 className="pt-1">Full Marks:</h5>
            </label>
            <input
              type="text"
              name="fullMarks"
              onChange={this.changeHandler}
              defaultValue={this.state.currentSubjectMarks}
              className="form-control"
            ></input>
          </form>
          <button onClick={this.edit} className="btn btn-warning">
            Save Changes
          </button>
